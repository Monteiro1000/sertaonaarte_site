document.addEventListener('DOMContentLoaded', function() {
    ...
    if (inscricaoForm) {
        inscricaoForm.addEventListener('submit', function(e) {
            ...
            let isValid = true;
            const formGroups = inscricaoForm.querySelectorAll('.form-group');
            ...
            function validarURL(id) {
                ...
                if (campo && campo.value && !urlRegex.test(campo.value)) {
                    campo.closest('.form-group').classList.add('error');
                    isValid = false;
                    return false;
                }
            ...
            // 1. Validar Identificacao do Proponente
            validarCampo('nomeResponsavel');
            validarCampo('nomeArtistico');
            validarCampo('cpfCnpj');
            validarCampo('cidadeUF');
            validarCampo('enderecoCompleto');
            validarCampo('telefoneContato');

            // Validar e-mail
            const emailContato = document.getElementById('emailContato');
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (emailContato && !emailRegex.test(emailContato.value)) {
                emailContato.closest('.form-group').classList.add('error');
                isValid = false;
            }

            // 2. Validar Dados da Proposta Musical
            validarCampo('nomeApresentacao');
            validarCampo('cidadeOrigem');
            validarSelect('formatoApresentacao');
            validarCampo('generosPredom');
            validarCampo('releaseArtistico');
            validarCampo('conceitoApresentacao');
            validarSelect('repertorioPrevisto');

            // 3. Validar Trajetoria e Experiencia
            validarCampo('apresentacoesRealizadas');
            validarCampo('participacaoEventos');

            // 4. Validar Material Artistico Obrigatorio
            validarCampo('videoAoVivo');
            validarCampo('linksAudio');
            validarCampo('redesSociais');
            validarURL('linkPortfolio');
            validarURL('linkFotos');
            validarCampo('linkFotos');

            // 5. Validar Ficha Tecnica
            validarCampo('integrantesFuncoes');
            validarCampo('mapaPalco');
            validarCampo('riderTecnico');
            ...
            if (isValid) {
                fetch(inscricaoForm.action, {
                    method: 'POST',
                    body: new FormData(inscricaoForm),
                    headers: {
